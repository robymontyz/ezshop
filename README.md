[![License](https://img.shields.io/github/license/robymontyz/ezshop?style=for-the-badge)](./LICENSE)
[![PoliTo](https://img.shields.io/badge/made_at-polito-blue?style=for-the-badge&logo=data:image/png;base64,)](www.polito.it)


# EZShop

> Shopping app written in Java + SQLite DB using Agile methodologies, including complete software documentation, during Software Engineering course at PoliTo 2020/21.

I developed this project during Software Engineering course held by Prof. M. Morisio, A.Y. 2020/2021, at Politecnico di Torino in a team of 4 people.

We developed a shopping application written in Java, using Agile methodology. Starting from a [requirement document](Deliverables/OfficialRequirements.md), we created the entire application, including documentation:

- [design document](Deliverables/DesignDocument.md)
- [GUI prototype using Balsamiq](Deliverables/GUIPrototype.md)
- test cases ([unit](Deliverables/UnitTestReport.md), [integration](Deliverables/IntegrationAPITestReport.md), acceptance)
- [timesheet](Deliverables/TimeSheet.md)
- [estimation document](Deliverables/Estimation.md)
- [project dashboard](Deliverables/DashBoard.md)
- [design assessment report using Structure101](Deliverables/DesignAssessmentDocument.md)

The project was managed through a Git repository and collaboration tools (Maven, Intellij IDEA - Code With Me, Visual Studio Code - Live Share, Slack, etc).

Main tasks:
• Producing software documentation using Markdown format with PlantUML extension;
• Designing, starting from a requirement document, a realistic, medium-sized, application using Agile methodologies;
• Analysis and formalization of functional and non-functional requirements of a software system;
• Testing, verification and validation of software through JUnit suite;
• Project management, configuration management, lifecycle analysis;
• Refining of social skills (team-working, team management) and soft skills (problem solving, project management, project planning).

## Contributing

Suggestions and improvements [are welcome](https://github.com/robymontyz/ezshop/issues).

Styles used:

- Versioning system standard: [Semantic Versioning](http://semver.org/)

- Commit messages standard: [Conventional Commits](https://www.conventionalcommits.org/en/v1.0.0/)

## Authors

- Roberto A. - [![GitHub](https://img.shields.io/github/followers/robymontyz?label=%40robymontyz&style=social)](https://github.com/robymontyz) - [![Twitter](https://img.shields.io/twitter/follow/robymontyz?label=%40robymontyz&style=social)](https://twitter.com/robymontyz)
- Michelangelo B.
- Roberto T.
- Gianvito M.

## License

This project is licensed and distributed under the [GNU General Public License v3.0](https://www.gnu.org/licenses/gpl.html) (**GPLv3**) - see the [LICENSE](./LICENSE) file for details.

-----

<p align="center">
    <img src="https://img.shields.io/badge/made_with-❤-ff69b4?style=for-the-badge"></a>
</p>
